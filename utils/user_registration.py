...
def show_user_login_page(language='zh'):
    ...
    user_name = st.text_input("姓名 / Name", key="login_name_final")
    user_inst = st.text_input("机构 / Institution", key="login_inst_final")
    mode = st.selectbox("模式 / Mode", ["新手 / Novice", "专家 / Expert"])
    
    if st.button("进入系统 / Enter", type="primary", use_container_width=True):
        if user_name and user_inst:
            selected_role = 'novice' if "新手" in mode else 'expert'
